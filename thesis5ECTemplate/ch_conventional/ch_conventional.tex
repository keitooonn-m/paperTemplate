...
\section{まえがき}
%----------------------------------------------
本章では，測定信号に対して，IVAを適用した結果をスペクトログラムに描画し，心拍推定アルゴリズムによって心拍推定結果を比較した結果を説明する．
心拍推定の比較には接触型ECG（心電図（Electrocardiogram: ECG））センサBiopac社Bioharnessの測定値を使用している．
...
%----------------------------------------------
STFTの窓長を確定するために一つの測定信号に対し4パターンの窓長に変更しSTFTを行った．実験条件はTable\ref{tab:iva}に示す．
%-%-%-%-%-%-%-%-%
\begin{table}[t]
...
\caption{Observation signal Back\_No2.}
\label{fig:siva32obs}
\end{figure}
%-%-%-%-%-%-%-%-%
...
\caption{Estimation signal Back\_No2 $\mathrm{FFT size}=32$.}
\label{fig:siva32est}
\end{figure}
%-%-%-%-%-%-%-%-%
...
\caption{Estimation signal Back\_No2 $\mathrm{FFT size}=64$.}
\label{fig:siva64est}
\end{figure}
%-%-%-%-%-%-%-%-%
...
\caption{Estimation signal Back\_No2 $\mathrm{FFT size}=96$.}
\label{fig:siva96est}
\end{figure}
%-%-%-%-%-%-%-%-%
...
\caption{Estimation signal Back\_No2 $\mathrm{FFT size}=128$.}
...
\begin{figure}[htbp]
    \begin{tabular}{cc}
      \begin{minipage}[t]{0.45\hsize}
        \centering
        \includegraphics[keepaspectratio, scale=0.8]{./ch_conventional/fig/hr_iva_32_ch1.pdf}
        \subcaption{Ch1}
        \label{hriva32ch1}
      \end{minipage} &
      \begin{minipage}[t]{0.45\hsize}
        \centering
        \includegraphics[keepaspectratio, scale=0.8]{./ch_conventional/fig/hr_iva_32_ch2.pdf}
        \subcaption{Ch2}
        \label{hriva32ch2}
      \end{minipage} \\
      ...
      \begin{minipage}[t]{0.45\hsize}
        \centering
        \includegraphics[keepaspectratio, scale=0.8]{./ch_conventional/fig/hr_iva_32_ch3.pdf}
        \subcaption{Ch3}
        \label{hriva32ch3}
      \end{minipage} &
      \begin{minipage}[t]{0.45\hsize}
        \centering
        \includegraphics[keepaspectratio, scale=0.8]{./ch_conventional/fig/hr_iva_32_ch4.pdf}
        \subcaption{Ch4}
        \label{hriva32ch4}
      \end{minipage} 
    \end{tabular}
     \caption{Heart rate estimation of observed signal with IVA applied.}
  \end{figure}
...
\section{まえがき}
%----------------------------------------------
本章では，測定信号に対して，基底数固定型ILRMAと基底数可変型ILRMA，$t$-ILRMAを適用した結果をスペクトログラムに描画し，心拍推定アルゴリズムによって心拍推定結果を比較した結果を説明する．
心拍推定の比較には接触型ECG（心電図（Electrocardiogram: ECG））センサBiopac社Bioharnessの測定値を使用している．
まず\ref{sec:conv:something}節では，何らかの分野で従来より用いられる何とかについて何とかを導入する．
\ref{sec:conv:somewhat}節では，なんとかについて述べる．
...
%-%-%-%-%-%-%-%-%
\begin{table}[t]
  \caption{Experimental conditions of ILRMA with fixed basis}
  \centering
  \begin{tabular}{cc} \hline
    Parameter & Value \\ \hline \hline
...
%-%-%-%-%-%-%-%-%
\begin{table}[t]
  \caption{Experimental conditions of ILRMA with variable basis}
  \centering
  \begin{tabular}{cc} \hline
    Parameter & Value \\ \hline \hline
    FFT length & 64 samples  \\ \hline
