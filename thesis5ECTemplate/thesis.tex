...
It is often fatal for a driver to \textcolor{red}{fall aaleep, have a sudden seizure}, or \textcolor{red}{lose} consciousness due to deterioration of physical condition while driving a \textcolor{red}{vehicle}. \textcolor{red}{Thus, monitoring the driver's condition in a vehicle is an crucial issue.}
\textcolor{red}{To address this issue}, this \textcolor{red}{thesis} deals with a system for constant\textcolor{red}{ly} monitoring the driver's heartbeat while driving utilizing a \textcolor{red}{contactless biometric sensor array} (hereafter referred to as radar sensor).
\textcolor{red}{Since the} radar sensor can measure the small displacement of the \textcolor{red}{driver's body surface}, a method to estimate the driver's heartbeat from the observed signal is discussed.
\textcolor{red}{The observed signal contains the driver's heartbeat signal but is contaminated with some noise including body motion and vibrations of the measurement system itself.}
Therefore, it is necessary to separate and extract only the \textcolor{red}{heartbeat signal} from the observed signals \textcolor{red}{to achieve the accurate heartbeat estimation.}
In this thesis, \textcolor{red}{I propose to apply blind source separation (BSS) techniques}, which has been \textcolor{red}{well} studied in the field of acoustic signal processing, \textcolor{red}{to the observed signal measured by the radar sensor.} \textcolor{red}{I experimentally show how much BSS can improve the estimation accuracy of the driver's heart rates.} From the experimental results, it is confirmed that the combination of \textcolor{red}{the radar sensor measurement} and BSS can \textcolor{red}{significantly} improve the accuracy of \textcolor{red}{the} heart rate estimation.
}
...
\textcolor{red}{この課題に取り組むために}，本論文では，運転中の運転者の心拍をレーダ非接触型生体センサアレイ（以後，レーダセンサと呼ぶ）を活用した常時モニタリングシステムを取り扱う．
\textcolor{black}{レーダセンサ\textcolor{red}{では，}運転差の体表面の微小変位を測定することができるため，この観測信号から運転者の心拍を推定する手法について検討する．
