...
It is often fatal for a driver to suffer from sleep, sudden seizure, or loss of consciousness due to deterioration of physical condition while driving a car. Therefore, it is one of the important issues to manage the driver's condition in some way while driving.
In order to solve this problem, this paper deals with a system for constant monitoring of the driver's heartbeat while driving, utilizing a radar non-contact biometric sensor array (hereafter referred to as radar sensor).
The radar sensor can measure the small displacement of the surface of the driver's body, and a method to estimate the driver's heartbeat from the observed signal is discussed.
However, there is a problem that the observed signal contains noise signals such as vibration of the measurement system itself and body motion in addition to the signal derived from the driver's heartbeat.
Therefore, it is necessary to separate and extract only the signals originating from the heartbeat from the observed signals in order to estimate the heartbeat with high accuracy.
In this thesis, we consider the application of blind source separation (BSS), which has been studied in the field of acoustic signal processing, to observation signals measured by radar sensors. We also experimentally investigate the improvement in estimation accuracy when the heartbeat-derived signals separated by BSS are input to a heartbeat estimation algorithm. From the experimental results, it is confirmed that the combination of radar sensor and BSS can improve the accuracy of heart rate estimation significantly.
}
...
この問題を解決するために，本論文では，運転中の運転者の心拍をレーダ非接触型生体センサアレイ（以後，レーダセンサと呼ぶ）を活用した常時モニタリングシステムを取り扱う．
\textcolor{red}{このとき，レーダセンサによって運転差の体表面の微小変位を測定することができるため，この観測信号から運転者の心拍を推定する手法について検討する．
しかしながら，観測信号には運転者の心拍に由来数信号以外にも，測定系自体の振動や体動等のノイズ信号が含まれる問題がある．
